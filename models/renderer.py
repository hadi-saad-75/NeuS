...
def extract_fields(bound_min, bound_max, resolution, query_func):
    # bound_min, bound_max are torch tensors on some device
...
def sample_pdf(bins, weights, n_samples, det=False):
    ...
    matched_shape = [inds_g.shape[0], inds_g.shape[1], cdf.shape[-1]]  # (B, n_samples, N)
...
class NeuSRenderer:
    ...
    def render_core_outside(self, rays_o, rays_d, z_vals, sample_dist, nerf, background_rgb=None):
        ...
        dists = torch.cat(
            [dists, torch.full_like(dists[..., :1], sample_dist, device=device)],
            dim=-1
        )
        ...
        dis_to_center = torch.linalg.norm(pts, ord=2, dim=-1, keepdim=True).clamp(1.0, 1e10)
        ...
        weights = alpha * torch.cumprod(
            torch.cat([torch.ones([batch_size, 1], device=device), 1.0 - alpha + 1e-7], dim=-1),
            dim=-1
        )[:, :-1]
        ...
        if background_rgb is not None:
            color = color + background_rgb.to(device) * (1.0 - weights.sum(dim=-1, keepdim=True))
    ...
    def render_core(self,
        ...
        gradients = sdf_network.gradient(pts_flat).squeeze(1)
        sampled_color = color_network(pts_flat, gradients, dirs_flat, feature_vector).reshape(batch_size, n_samples, 3)

        inv_s = deviation_network(torch.zeros([1, 3], device=device))[:, :1].clamp(1e-6, 1e6)
        inv_s = inv_s.expand(batch_size * n_samples, 1)

        true_cos = (dirs_flat * gradients).sum(dim=-1, keepdim=True)
        ...
        pts_norm = torch.linalg.norm(pts_flat, ord=2, dim=-1, keepdim=True).reshape(batch_size, n_samples)
